...
@database = SQLite3::Database.new("db/twiter.sqlite3")
...
def addTweet(usuario, mensaje, id_busqueda)
	found = @database.execute( "select * from results where usuario = ? and mensaje = ?",
				usuario,
				mensaje)
		
	if found.empty? == true then
		@database.execute("insert into results (usuario,mensaje,id_search) values (
				'#{usuario}',
				'#{mensaje}',
				 #{id_busqueda})")
	end
...
found = @database.execute( "select id,palabra from searches")
found.each do |texto|
